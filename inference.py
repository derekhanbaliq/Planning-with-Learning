...
from controllers.kinematic_mpc import mpc_config, State, KMPCPlanner
...
from utils.waypoint_loader import WaypointLoader, pi_2_pi
...
def main():
    ...
    if method == 'pure_pursuit':
        ...
        controller = KMPCPlanner(debug=True, waypoints=np.column_stack((waypoints.x, waypoints.y, waypoints.v,
                                                                        pi_2_pi(waypoints.θ))))
    ...
    init_pos = np.array([[4.0, 5.0, -np.pi]])
    ...
    while not done:
        ...
        if method == 'kinematic_mpc':
            if int(lap_time * 100) % 20 == 0:  # 100 Hz
                ...
                renderer.horizon_traj = mpc_pred_traj  # yellow
